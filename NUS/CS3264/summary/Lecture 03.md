...
- **MLE loss**: $\mathcal{L}_{\text{MLE}}(w) = -\left[\sum_{n=1}^{N} t_n \log y_n + (1-t_n) \log(1-y_n)\right]$
- **MAP loss**: $\mathcal{L}_{\text{MAP}}(w) = \mathcal{L}_{\text{MLE}}(w) + \frac{\alpha}{2} \|w\|_2^2$
...
$$
\log p(w \mid X, \mathbf{t})
\propto - \mathcal{L}_{\text{MLE}}(w) - \frac{\alpha}{2}\|w\|_2^2 + \text{const}
$$
